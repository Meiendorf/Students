...
namespace Students.Controllers
{
    ...
    public class AccountsController : Controller
    {
        ...
        private readonly IConfigurationSection jwtSec;
            ...
            SignInManager<User> signInManager, RoleManager<IdentityRole> roleManager, IHostingEnvironment env)
        {
            ...
            jwtSec = _configuration.GetSection("Jwt");
            _env = env;
        }
        ...
        private async Task<object> GenerateJwtTokenAsync(string email, User user)
        {
            ...
            var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(jwtSec["JwtKey"]));
            var creds = new SigningCredentials(key, SecurityAlgorithms.RsaSha256);
            var expires = DateTime.Now.AddDays(Convert.ToDouble(jwtSec["JwtExpireDays"]));
        ...
        private string GenerateMpassToken()
        {
            var claims = new List<Claim>
            {
            ...
            X509Certificate2 cert = new X509Certificate2(Path.Combine(_env.ContentRootPath, jwtSec["JwtSertificate"]), jwtSec["JwtPass"]);
